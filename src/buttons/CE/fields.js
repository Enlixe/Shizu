...
module.exports = {
  ...
  execute(interaction, client) {
    ...
    let msgEmbed = new EmbedBuilder()
      .setColor("F4D58D")
      .setDescription(`**setting \`${modifying}\`:**`);
    interaction.reply({
      embeds: [msgEmbed],
    });
    ...
    buttonCollector.on("collect", (btnInt) => {
      ...
      if (btnInt.component.customId === "set") {
          ...
          return interaction.editReply({
            embeds: [
              EmbedBuilder.from(msgEmbed)
                .setColor("DarkRed")
                .setDescription(`**Max fields have been set**`),
            ],
          });
        }
        ...
        interaction.editReply({
          embeds: [
            EmbedBuilder.from(msgEmbed).setDescription(`**Added: ✅ **`),
          ],
        });
      }
    });
    ...
    collector.on("collect", (m) => {
      ...
      if (modifying === "name") {
        ...
        if (m.content.length > 256)
          return error(interaction, "name is too long", m);
      ...
      if (modifying === "value") {
        ...
        if (m.content.length > 1024)
          return error(interaction, "value is too long", m);
