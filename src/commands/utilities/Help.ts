import { ActionRowBuilder, ChatInputCommandInteraction, ComponentType, EmbedBuilder, Interaction, PermissionsBitField, StringSelectMenuBuilder } from "discord.js";
    ...
    async Execute(interaction: ChatInputCommandInteraction) {
        ...
        if (this.commandCache.size === 0) {
            try {
                ...
                return interaction.reply({ content: "Failed to fetch commands.", ephemeral: true });
            }
        }

        const categories = this.getCategories();

        const embed = this.client.config.createEmbed()
        ...
        collector?.on("collect", async (interaction) => {
            ...
            const categoryEmbed = this.client.config.createEmbed()
                .setTitle(`${EMOJIS[directory.toLowerCase()]} ${this.formatString(directory)} commands`)
