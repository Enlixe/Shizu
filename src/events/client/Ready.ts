...
export default class Ready extends Event {
  ...
  async Execute() {
    ...
    try {
      ...
      this.client.logger.error(`Error in Ready event: ${error.message}`, [
        "Event",
        "Ready",
      ]);
    }
  ...
  private async registerCmd() {
    try {
      const clientId = this.client.devMode
        ? this.client.config.dev_client_id
        : this.client.config.client_id;
      ...
      const devCommands: any = await rest.put(
        Routes.applicationGuildCommands(clientId, this.client.config.dev_guild),
        {
          body: this.GetJson(this.client.commands.filter((cmd) => cmd.dev)),
        }
      );
      ...
      this.client.logger.error(`Error registering commands: ${error.message}`, [
        "Event",
        "Ready",
      ]);
    }
    ...
    try {
      ...
      const updateActivity = (): void => {
        try {
          ...
          this.client.logger.error(`Failed to set activity: ${err.message}`, [
            "Event",
            "Ready",
          ]);
        }
      ...
      this.client.logger.error(`Error setting activity: ${error.message}`, [
        "Event",
        "Ready",
      ]);
    }
