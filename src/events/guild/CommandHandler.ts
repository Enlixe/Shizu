  ...
  async Execute(interaction: ChatInputCommandInteraction) {
      ...
      return interaction.reply({
        embeds: [
          new EmbedBuilder()
            .setColor("Red")
            .setDescription(
              `❌ Please wait another \`${(((timestamps?.get(interaction.user.id) || 0) + cooldownAmount - now) / 1000).toFixed(1)}\` seconds, before reusing the \`${cmd.name}\` command`
            ),
        ],
        flags: 64,
      });
    ...
    try {
      ...
      this.client.logger.error(
        `Error executing command ${cmd.name}: ${error.message}`,
        ["CommandHandler"]
      );
      interaction.reply({
